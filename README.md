...
- **作業時間の短縮**：担当者ヒアリングに基づく試算では、動画の字幕作成にかかる作業時間全体を15.8%削減
...
デスクトップアプリ自体は一般的なPCで動作し、**GPUは必須ではありません**（動画再生と音声抽出（同梱 FFmpeg）が動く程度のスペックで十分です）。目安は次のとおりです。

| 項目 | 目安 |
|---|---|
| 対応OS | Windows 10/11 (x64) / macOS (Apple Silicon) / Linux x64 (AppImage) / Fedora (RPM) |
| CPU | 64bit（x64 または Apple Silicon） |
| メモリ (RAM) | 8GB 以上（長時間の講義動画や大きなプロジェクトを扱うなら 16GB 推奨） |
| ストレージ | アプリ本体は数百MB程度（同梱 FFmpeg 込み）＋ 扱う動画ファイル分の空き |
| ディスプレイ | 横 1280px 以上を推奨（左右2ペインのUIのため） |
| GPU | アプリ本体には不要（このPCで書きおこしする場合のみ GPU が必要 → 下記） |
| Windows のみ | Microsoft Edge WebView2 ランタイム（Windows 10/11 には通常同梱） |
