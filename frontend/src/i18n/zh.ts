...
export const zh: LocaleStrings = {
  ...
  toast: {
    ...
    exportCsv: 'CSV 已导出',
    exportJson: 'JSON 已导出',
    exportAdminSettings: '共享设置 JSON 已导出',
    exportGlossaryLog: '词汇表生成日志已导出',
    importGlossary: ({ added = 0, updated = 0 }) => `词汇表已导入：新增 ${added} 条，更新 ${updated} 条`,
    importAdminSettings: '共享设置 JSON 已导入',
    saveSettings: '设置已保存',
  ...
  srtFormatTarget: '仅原文',
  srtFormatBoth: '两者',
  srtFormatSelectTitle: '选择 SRT 导出格式（字幕 / 原文 / 两者）',
  restored: '已恢复上次的工作',
  videoLoadChoiceTitle: '要加载视频吗？',
  videoLoadChoiceDesc: (name, count) => `将加载“${name}”。请选择重置当前 ${count} 个字幕块，或保留字幕并仅替换视频。`,
  videoLoadReset: '重置字幕并加载',
  videoLoadKeep: '保留字幕并加载',
  videoLoadCancel: '取消',
  ...
  shortcuts: [
    {
      category: '编辑',
      items: [
        { keys: ['点击（原文/字幕文本）'], desc: '进入编辑模式' },
        { keys: ['Enter'], desc: '插入换行（按设置页的单行字数上限调整）' },
    ...
    },
    {
      category: '合并',
      items: [
        { keys: ['↕ 菜单'], desc: '选择与上一块或下一块合并（已审批的块除外）' },
    ...
    },
    {
      ...
      items: [
        ...
        { keys: ['Enter（编辑时间时）'], desc: '保存时间编辑' },
        { keys: ['Esc（编辑时间时）'], desc: '取消时间编辑' },
  ...
  settingsTranslatorProvider: 'AI 提供方',
  settingsTranslatorProviderOpenAi: 'OpenAI',
  settingsTranslatorProviderGemini: 'Gemini',
  settingsTranslatorProviderLocalOpenAi: 'OpenAI兼容',
  settingsOpenAiApiKey: 'OpenAI API Key',
  settingsLocalOpenAiApiKey: 'API Key（可选）',
  ...
  settingsSubtitleQualityTitle: '字幕质量参数',
  settingsEnMaxCharsPerLine: '每行最大字符数',
  settingsEnMaxLines: '最大行数',
  settingsEnMaxTotalChars: '所有行合计最大字符数',
  settingsEnMaxCps: '最大CPS（字符/秒）',
  settingsSubtitleMinDuration: '字幕最短显示时间（秒）',
  settingsSubtitleMaxDuration: '字幕最长显示时间（秒）',
  settingsQualityCorrectionThreshold: '补正质量阈值（Embedding距离）',
  settingsPipelineThresholdsTitle: '流水线阈值',
  settingsPipelineShortDurationSec: '过短判断（秒）',
  settingsPipelineLongDurationSec: '过长判断（秒）',
  settingsPipelineMergedLongDurationSec: '合并后过长判断（秒）',
  settingsPipelineVerboseEnRatio: '冗余判断 EN/JA比',
  settingsPipelineOverCompressedRatio: '过度压缩 EN/JA比（下限）',
  settingsPipelineOverCompressedJaChars: '过度压缩 JA字数（下限）',
  settingsPipelineSlowCps: '语速过慢 CPS（下限）',
  settingsPipelineMaxExpandPerBlock: '展开重试上限（per block）',
  settingsPipelineMaxCompressPerBlock: '压缩重试上限（per block）',
  settingsPipelineMaxPhase2Retries: 'Phase 2 循环上限',
  settingsPipelineMergeContinuationEnabled: '未完结句合并（Phase1）',
  settingsPipelineMergeContinuationMaxGapSec: '可合并相邻间隔上限（秒）',
  settingsPipelineMergeContinuationMaxDurationSec: '合并后最大时长（秒）',
  ...
  settingsExpandModel: '展开模型',
  settingsCompressPromptOverride: '压缩提示词覆盖',
  settingsExpandPromptOverride: '展开提示词覆盖',
  settingsRefreshModels: '刷新模型列表',
  settingsRefreshModelsLoading: '获取中...',
  settingsRefreshModelsError: '获取失败',
  settingsSemanticCheckTitle: '语义检查',
  settingsSemanticCheckMode: '模式',
