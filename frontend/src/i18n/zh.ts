...
export const zh: LocaleStrings = {
  ...
  toast: {
    ...
    importProjectJson: '项目 JSON 已加载',
    projectImportError: ({ error = '' }) => `无法加载项目 JSON：${error}`,
    projectVersionTooNew: ({ found = '', supported = '' }) => `此项目使用较新的格式（v${found}；当前最高支持 v${supported}）`,
    projectExportError: ({ error = '' }) => `无法保存项目 JSON：${error}`,
    workLogWriteWarning: ({ error = '' }) => `工作日志写入失败。内存中的历史仍会包含在项目 JSON 中：${error}`,
    exportCsv: 'CSV 已导出',
    exportJson: 'JSON 已导出',
  ...
  settingsPipelineMaxExpandPerBlock: '展开重试上限（每块）',
  settingsPipelineMaxCompressPerBlock: '压缩重试上限（每块）',
  settingsPipelineMergeContinuationEnabled: '将未完结的句子与下一条字幕合并',
