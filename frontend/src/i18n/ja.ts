...
export const ja: LocaleStrings = {
  ...
  toast: {
    ...
    importProjectJson: 'プロジェクトJSONを読み込みました',
    projectImportError: ({ error = '' }) => `プロジェクトJSONを読み込めませんでした: ${error}`,
    projectVersionTooNew: ({ found = '', supported = '' }) => `このプロジェクトは新しい形式です（v${found}、対応はv${supported}まで）`,
    projectExportError: ({ error = '' }) => `プロジェクトJSONを保存できませんでした: ${error}`,
    workLogWriteWarning: ({ error = '' }) => `作業ログの保存に失敗しました。プロジェクトJSONにはメモリ上の履歴を含めます: ${error}`,
  ...
  settingsPipelineMaxExpandPerBlock: '展開リトライ上限（1ブロックあたり）',
  settingsPipelineMaxCompressPerBlock: '圧縮リトライ上限（1ブロックあたり）',
  settingsPipelineMergeContinuationEnabled: '未完結な文を次の字幕と結合する',
